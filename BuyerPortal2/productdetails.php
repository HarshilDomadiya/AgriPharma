...
<nav class="navbar navbar-expand-xl ">
                            ...
                            <div class='text-center mt-2 ''>
                                ...
                                <form actions='' method='post'>
                                    ...
                                    <div class='row'>
                                        ...
                                        <div class='col-12 col-xl-6 col-lg-6 col-md-6 col-sm-12'> 
                                            <button name='saveforlater' type='submit' class='btn btn-warning border-secondary saveforlater' style='color:black'><b>Save For later</b><img src='saveforlater4.png' class='ml-1 mb-1'></button>
                                        </div>
                                    </div>
                                </form>
    ...
    if (isset($_POST['saveforlater'])) {
        ...
        if (isset($_SESSION['phonenumber'])) {
            ...
            $check_saved = "SELECT * FROM saveforlater WHERE phonenumber = $sess_phone_number AND product_id='$product_id'";
            $run_check = mysqli_query($con, $check_saved);
            
            if (mysqli_num_rows($run_check) > 0) {
                echo "<script>window.alert('This product is already in your saved items!');</script>";
            } else {
                // Insert into saveforlater table
                $insert_saved = "INSERT INTO saveforlater (product_id, phonenumber, qty) VALUES ('$product_id', '$sess_phone_number', '$qty')";
                $run_insert = mysqli_query($con, $insert_saved);
                
                if ($run_insert) {
                    echo "<script>window.alert('Product saved for later!');</script>";
